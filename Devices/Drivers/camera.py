...
class CameraException(Exception):
    def __init__(self, camera, message='', code=-1, stop=True):
        i_error_codes = ueye_codes.invert_dict(
            ueye_codes.get_codes(ueye_codes.error_codes)
        )
...
class ueye(object):
    ...
    def configure_camera(self, mode='RGB8', **kwargs):
        ...
        if err != pue.IS_SUCCESS:
            raise CameraException(
                self.cam, 'ueye>configure_image>set_display_mode>', err
            )
        ...
        if err != pue.IS_SUCCESS:
            raise CameraException(
                self.cam, 'ueye>configure_image>set_hardware_gain>', err
            )
        err = pue.is_SetHardwareGamma(self.cam, pue.IS_SET_HW_GAMMA_OFF)
        if err != pue.IS_SUCCESS:
            raise CameraException(
                self.cam, 'ueye>configure_image>set_hardware_gamma>', err
            )
        '''
        black_level = 70
    ...
    def allocate_memory(self):
        ...
        err = pue.is_AllocImageMem(
            self.cam, c_width, c_height, c_pixel_bits, self.ppcImgMem, self.pid
        )
        if err != pue.IS_SUCCESS:
