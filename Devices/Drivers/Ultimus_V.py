...
class Ultimus_V_Pump:
    ...
    VAC_UNITS = {'kpa': 0, 'inh2o': 1, 'inhg': 2, 'mmhg': 3, 'torr': 4}

    PRESS_UNITS = {'psi': 0, 'bar': 1, 'kpa': 2}

    VAC_MULT = {'kpa': 100, 'inh2o': 10, 'inhg': 100, 'mmhg': 10, 'torr': 10}

    PRESS_MULT = {'psi': 10, 'bar': 1000, 'kpa': 10}
    ...
    def transmit_command(self, command):
        ...
        if self.no_pump:
            ...
            if self.pump.isOpen():
                ...
                if self.debug:
                    print(
                        'Cannot send command: {} , pump is not open'.format(
    ...
    def set_vacuum_units(self, units):
        ...
        if units.lower() in Ultimus_V_Pump.VAC_UNITS:
            if self.vacuum_units is not units.lower():
                r = self.transmit_command(
                    self.format_command(
                        "E7  {:02}".format(Ultimus_V_Pump.VAC_UNITS[units.lower()])
                    )
                )
                if r == 1:
    ...
    def set_pressure_units(self, units):
        ...
        if units.lower() in Ultimus_V_Pump.PRESS_UNITS:
            if self.pressure_units is not units.lower():
                r = self.transmit_command(
                    self.format_command(
                        "E6  {:02}".format(Ultimus_V_Pump.PRESS_UNITS[units.lower()])
                    )
                )
                if r == 1:
    ...
    def get_vacuum_units(self):
        ...
        if "VU".encode("utf-8") in raw:
            ...
            units = [key for key, v in Ultimus_V_Pump.VAC_UNITS.items() if v == value][
                0
            ]
    ...
    def get_pressure_units(self):
        ...
        if "PU".encode("utf-8") in raw:
            ...
            # need to do reverse lookup in dictionary
            units = [
                key for key, v in Ultimus_V_Pump.PRESS_UNITS.items() if v == value
            ][0]
