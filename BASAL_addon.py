...
bl_info = {
    "name": "BASAL Addon",
    "author": "Nicholas Arn, fork from S0AndS0 (no longer maintained)",
    "version": (2, 4),
    "blender": (2, 93, 4),
    "location": "View3D > Sidebar > 3D-Printing",
    "description": "Enables translation to GCode without leaving Blender & uploading to OctoPrint or Repetier server(s) & using the BASAL (Blender APE Slic3r APE) loop ",
    "warning": "Server interactions are in beta",
    "origonal wiki_url (no longer maintained)": "https://s0ands0.github.io/3D_Printing/blender/addons/3dprint-short-cuts/readme.html",
...
class Slic3r(object):
    ...
    def slice_stl(self, stl_path=None, gcode_path=None):
        ...
        if os.path.exists(os.path.join(bpy.context.scene.slic3r_exec_dir, bpy.context.scene.slic3r_exec_name)):
            self.slic3r_exec_path = os.path.join(bpy.context.scene.slic3r_exec_dir, bpy.context.scene.slic3r_exec_name)
        else:
            self.slic3r_exec_path = bpy.context.scene.slic3r_exec_name
        if os.path.exists(os.path.join(bpy.context.scene.curaengine_exec_dir, bpy.context.scene.curaengine_exec_name)):
            self.curaengine_exec_path = os.path.join(bpy.context.scene.curaengine_exec_dir, bpy.context.scene.curaengine_exec_name)
        else:
            self.curaengine_exec_path = bpy.context.scene.curaengine_exec_name
        if os.path.exists(os.path.join(bpy.context.scene.curl_exec_dir, bpy.context.scene.curl_exec_name)):
            self.curl_exec_path = os.path.join(bpy.context.scene.curl_exec_dir, bpy.context.scene.curl_exec_name)
        else:
            self.curl_exec_path = bpy.context.scene.curl_exec_name
        ...
        #slic3r_ops = [object.slic3r_exec_path]
        ...
        #slice_stl_output = slic3r_output
...
class local_slice_button(Operator):
    ...
    def execute(self, context):
        ...
        formated_output = Formatted_output.return_output(op_output)
        apefile = context.scene.basal_home_directory_monolith
        #try:
            #exec(open(apefile).read())
        #except:
            #pass
        for info in formated_output:
            self.report({'INFO'}, info)
...
class start_ape_button(Operator):
    """start ape in background"""
    bl_idname = 'object.start_ape_button'
    bl_label = 'Start APE Loop'

    def execute(self, context):
        #file_com = self.FileCom(context)
        stayinloop = 1
        apefile = context.scene.basal_home_directory_monolith
        modtime = 0
        lastmodtime = 1
        if not context.selected_objects:
            raise Exception('Please select the object you wish to iterate over first')
        obfile = bpy.context.selected_objects[0].name
        if os.name == 'posix':
            path = os.path.expanduser('~')+'/Print_With_APE/'+obfile+'.txt'
            cmdpath = os.path.expanduser('~')+'/Print_With_APE/cmd'+obfile+'.txt'
            cmdpath.encode('unicode_escape')
        elif os.name == 'nt':
            path = os.path.expanduser('~')+'\Print_With_APE\ '
            path = path.rstrip(path[-1])
            path = path+obfile+'.txt'
            cmdpath = os.path.expanduser('~')+'\Print_With_APE\cmd'+obfile+'.txt'
            cmdpath.encode('unicode_escape')
        while stayinloop == 1:
            if apefile:
                modtime = os.path.getmtime(cmdpath)
            ...
            if modtime == lastmodtime:
                ...
                if os.path.exists(cmdpath):
                    ...
                    for i in fileread:
                        ...
                        inum = inum+1
                    for lstnum in range(len(lstoflsts)):
                        
                        
                        
                        if lstoflsts[lstnum][0] == 'continue:':
                            continuecmd = float(lstoflsts[lstnum][1])
                        if lstoflsts[lstnum][0] == 'duplicate_move_x:':
                            bpy.ops.object.duplicate_move(OBJECT_OT_duplicate={"linked":False, "mode":'TRANSLATION'}, TRANSFORM_OT_translate={"value":(float(lstoflsts[lstnum][1]), -0, -0), "orient_type":'GLOBAL', "orient_matrix":((1, 0, 0), (0, 1, 0), (0, 0, 1)), "orient_matrix_type":'GLOBAL', "constraint_axis":(True, False, False), "mirror":True, "use_proportional_edit":False, "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "use_proportional_connected":False, "use_proportional_projected":False, "snap":False, "snap_target":'CLOSEST', "snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "gpencil_strokes":False, "cursor_transform":False, "texture_space":False, "remove_on_cancel":False, "release_confirm":False, "use_accurate":False})
                            newobjid = bpy.context.active_object.name
                            bpy.data.objects[newobjid].select_set(True)
                            bpy.context.view_layer.objects.active = bpy.data.objects[newobjid]
                            
                        if lstoflsts[lstnum][0] == 'duplicate_move_y:':
                            bpy.ops.object.duplicate_move(OBJECT_OT_duplicate={"linked":False, "mode":'TRANSLATION'}, TRANSFORM_OT_translate={"value":(-0, float(lstoflsts[lstnum][1]), -0), "orient_type":'GLOBAL', "orient_matrix":((1, 0, 0), (0, 1, 0), (0, 0, 1)), "orient_matrix_type":'GLOBAL', "constraint_axis":(False, True, False), "mirror":True, "use_proportional_edit":False, "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "use_proportional_connected":False, "use_proportional_projected":False, "snap":False, "snap_target":'CLOSEST', "snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "gpencil_strokes":False, "cursor_transform":False, "texture_space":False, "remove_on_cancel":False, "release_confirm":False, "use_accurate":False})
                            newobjid = bpy.context.active_object.name
                            bpy.data.objects[newobjid].select_set(True)
                            bpy.context.view_layer.objects.active = bpy.data.objects[newobjid]
                        if lstoflsts[lstnum][0] == 'duplicate_move_z:':
                            bpy.ops.object.duplicate_move(OBJECT_OT_duplicate={"linked":False, "mode":'TRANSLATION'}, TRANSFORM_OT_translate={"value":(-0, -0, float(lstoflsts[lstnum][1])), "orient_type":'GLOBAL', "orient_matrix":((1, 0, 0), (0, 1, 0), (0, 0, 1)), "orient_matrix_type":'GLOBAL', "constraint_axis":(False, False, True), "mirror":True, "use_proportional_edit":False, "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "use_proportional_connected":False, "use_proportional_projected":False, "snap":False, "snap_target":'CLOSEST', "snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "gpencil_strokes":False, "cursor_transform":False, "texture_space":False, "remove_on_cancel":False, "release_confirm":False, "use_accurate":False})
                            newobjid = bpy.context.active_object.name
                            bpy.data.objects[newobjid].select_set(True)
                            bpy.context.view_layer.objects.active = bpy.data.objects[newobjid]
                        if lstoflsts[lstnum][0] == 'scale_u:':
                            bpy.ops.transform.resize(value=(float(lstoflsts[lstnum][1]), float(lstoflsts[lstnum][1]), float(lstoflsts[lstnum][1])), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(False, True, False), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                        if lstoflsts[lstnum][0] == 'scale_x:':
                            bpy.ops.transform.resize(value=(float(lstoflsts[lstnum][1]), 1, 1), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(True, False, False), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                        if lstoflsts[lstnum][0] == 'scale_y:':
                            bpy.ops.transform.resize(value=(1, float(lstoflsts[lstnum][1]), 1), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(False, True, False), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                        if lstoflsts[lstnum][0] == 'scale_z:':
                            bpy.ops.transform.resize(value=(1, 1, float(lstoflsts[lstnum][1])), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(False, False, True), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)

                        if lstoflsts[lstnum][0] == 'prop_edit_+z_x:':
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = True
                            bpy.ops.mesh.select_all(action='DESELECT')
                            selectvert = bpy.context.active_object.data.vertices
                            lastcoz = -5000000000000
                            numv = 0
                            ...
                            for vert in vertices:
                                if vert.index == myvert:
                                    vert.select = True
                                else:
                                    vert.select = False
                            bmesh.update_edit_mesh(me, True) 
                            floatlist = float(lstoflsts[lstnum][1])
                            bpy.ops.transform.translate(value=(floatlist, 0, 0), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(True, False, False), mirror=True, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                            bpy.ops.object.editmode_toggle()
                        ...
                        if lstoflsts[lstnum][0] == 'prop_edit_-z_x:':
                            ...
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = False
                        if lstoflsts[lstnum][0] == 'prop_edit_+z_y:':
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = True
                            bpy.ops.mesh.select_all(action='DESELECT')
                            selectvert = bpy.context.active_object.data.vertices
                            ...
                            bmesh.update_edit_mesh(me, True) 
                            floatlist = float(lstoflsts[lstnum][1])
                            bpy.ops.transform.translate(value=(0, floatlist, 0), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(False, True, False), mirror=True, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = False                        
                        if lstoflsts[lstnum][0] == 'prop_edit_-z_y:':
                            bpy.ops.object.editmode_toggle()
                            ...
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = False                        
                        if lstoflsts[lstnum][0] == 'prop_edit_+z_z:':
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = True
                            bpy.ops.mesh.select_all(action='DESELECT')
                            selectvert = bpy.context.active_object.data.vertices
                            ...
                            floatlist = float(lstoflsts[lstnum][1])
                            bpy.ops.transform.translate(value=(0, 0, floatlist), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(False, False, True), mirror=True, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
                            bpy.ops.object.editmode_toggle()
                            bpy.context.scene.tool_settings.use_proportional_edit = False
                        if lstoflsts[lstnum][0] == 'prop_edit_-z_z:':
                            bpy.ops.object.editmode_toggle()
                        ...
                        if lstoflsts[lstnum][0] == 'boolean_subtract:':
                            modobj = bpy.context.active_object.name
                            mod = bpy.context.active_object.modifiers.new("Boolean", type='BOOLEAN')
                            mod.operation = 'DIFFERENCE'
                            mod.object = bpy.data.objects[lstoflsts[lstnum][1]]
                            bpy.ops.object.modifier_apply(modifier=mod.name)
                            
                        if lstoflsts[lstnum][0] == 'select_new_object:':
                            newobjid = lstoflsts[lstnum][1]
                            bpy.ops.object.select_all(action = 'DESELECT')
                            bpy.data.objects[newobjid].select_set(True)
                            bpy.context.view_layer.objects.active = bpy.data.objects[newobjid]
                        if lstoflsts[lstnum][0] == 'hide_object:':
                            if lstoflsts[lstnum][1] == 'selected':
                                bpy.context.active_object.hide_set(True)
                            else:
                                bpy.ops.object.select_all(action = 'DESELECT')
                                bpy.data.objects[lstoflsts[lstnum][1]].select_set(True)
                                bpy.context.view_layer.objects.active = bpy.data.objects[lstoflsts[lstnum][1]]
                                bpy.context.active_object.hide_set(True)
                    
                    #for objtmp in bpy.context.scene.objects:
                        #if objtmp.name == newobjid:
                            #bpy.ops.object.select_all(action = 'DESELECT')
                            #bpy.data.objects[newobjid].select_set(True)
                else:
                    continuecmd = 0
            if continuecmd == 1 or continuecmd == 2:
                if modtime == lastmodtime:
                    time.sleep(1)
                else:
                    lastmodtime = modtime
                    local_slice_button.execute(self, context)
            if continuecmd == 0 or continuecmd == 2:
                stayinloop = 0
            bpy.ops.object.select_all(action = 'DESELECT')
            bpy.data.objects[objid].select_set(True)
            bpy.context.view_layer.objects.active = bpy.data.objects[objid]
        info = ('Finished')
        self.report({'INFO'}, info)
        return {'FINISHED'}
...
class basal_settings(PropertyGroup):
    Scene = bpy.types.Scene
    Scene.basal_home_directory_monolith = StringProperty(
        name='basal_home_directory_monolith',
        default='',
        description='Location of APE engine monolith used to start BASAL',
        subtype='FILE_PATH',
    )
...
class print_server_connection_panel(Panel):
    ...
    def draw(self, context):
        ...
        if 'OctoPrint' in scene.preferred_print_server:
            ...
            layout.prop(scene, 'curl_exec_dir', text='Directory of Executable')
            layout.prop(scene, 'curl_exec_name', text='Name of Executable')
        elif 'BASAL' in scene.preferred_print_server:
            layout.prop(scene, 'basal_home_directory_monolith', text='APE engine monolith location')
            layout.operator('object.start_ape_button', text='Start APE Loop')
...
def menu_func(self, context):
    ...
    layout.operator(print_server_connection_panel.bl_idname)
    layout.operator(print_server_slicer_panel.bl_idname)
    layout.operator(preview_webcam_panel.bl_idname)
