...
class InitialImages(Core.Procedure):
    ...
    def Plan(self):
        ...
        noz_axismask = self.apparatus.getValue(
        ...
        cam_refpoint = self.apparatus.getValue(
            ['information', 'alignments', cameraname + '@start']
        )
        ...
        point1 = self.apparatus.getValue(
            ['information', 'toolpaths', 'parameters', 'point1']
        )
        point2 = self.apparatus.getValue(
            ['information', 'toolpaths', 'parameters', 'point2']
        )
        ...
        self.pmove.Do(
            {
                'refpoint': noz_refpoint,
                'relpoint': relpoint,
                'priority': priority,
                'axismask': noz_axismask,
            }
        )
        ...
        self.pmove.Do(
            {
                'refpoint': cam_refpoint,
                'relpoint': relpoint,
                'priority': priority,
                'axismask': cam_axismask,
            }
        )
        self.DoEproc(motionname, 'Run', {})
        ...
        # Take a picture
        filename = (
            'Data\\' + str(round(time.time())) + samplename + '_startnozalign' + '.tif'
        )
        ...
        refpoint = self.apparatus.getValue(
            ['information', 'alignments', 'safe' + noz_axismask['Z']]
        )
        ...
        # Take a picture
        filename = (
            'Data\\' + str(round(time.time())) + samplename + '_endnozalign' + '.tif'
        )
        self.apparatus.setValue(
            [
                'information',
        ...
        refpoint = self.apparatus.getValue(
            ['information', 'alignments', 'safe' + cam_axismask['Z']]
        )
