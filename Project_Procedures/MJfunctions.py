...
def LABsegmenter(imgf, imgi, samplename, path, save):
	...
	imgf = cv2.cvtColor(imgf, cv2.COLOR_RGB2BGR) #no clue why RGB gets red in as BGR but anyways...
	LAB_imgf = skimage.color.rgb2lab(imgf) #convert to LAB
	#LAB_img = cv2.convertScaleAbs(LAB_img) not necessary
	
	#repeat for initials
	imgi = cv2.cvtColor(imgi, cv2.COLOR_RGB2BGR) #no clue why RGB gets red in as BGR but anyways...
	LAB_imgi = skimage.color.rgb2lab(imgi) #convert to LAB
	
	
	threshf = (LAB_imgf[:,:,0] > 0) & (LAB_imgf[:,:,0]< 31) & (LAB_imgf[:,:,1] > 0) & (LAB_imgf[:,:,1]  < 100) & (LAB_imgf[:,:,2] > -106) & (LAB_imgf[:,:,2] <-3)
	
	threshi = (LAB_imgi[:,:,0] > 0) & (LAB_imgi[:,:,0]< 31) & (LAB_imgi[:,:,1] > 0) & (LAB_imgi[:,:,1]  < 100) & (LAB_imgi[:,:,2] > -106) & (LAB_imgi[:,:,2] <-3)
	...
	subtracted = morphology.remove_small_objects(np.array(subtracted, bool), 10000, connectivity = 1)
	...
	if save == True:
		...
		plt.imsave('Data\\' + str(round(time.time())) + samplename + '_LABseg' + '.tif', binary_img)
...
def gap_finder(imgi, imgf, binary_img, gap_padding, samplename, path, save):
	...
	try:
		...
		gapThresh = (gap_colored[:,:,0] == 0) & (gap_colored[:,:,1] == 255) & (gap_colored[:,:,2] == 255)
		
		if gap_padding == 0:
			gapPadded= gapThresh
			
		elif gap_padding != 0:
			ind_mat = np.arange(0, gapThresh.shape[0]*gapThresh.shape[1]).reshape((gapThresh.shape))
			...
			for row in range(gap_inds.shape[0]):
				...
				far_right.append((np.unravel_index(row_indices.argmax(), gapThresh.shape[1]))[0])
				
				row_indices[row_indices == 0 ] = gapThresh.shape[0]*gapThresh.shape[1] + 5 #make the zeros something bigger than we could ever possibly get just so zero is never the minimum
				#far_left.append(np.min(row_indices))
				far_left.append((np.unravel_index(row_indices.argmin(), gapThresh.shape[1]))[0])
			...
			for row in range(gap_inds.shape[0]):
				...
				gapExpanded[row, (far_left[row] - gap_padding):(far_right[row]+gap_padding)] = 1
				
			
			
			
			gapPadded_f = imgf.astype(np.uint8) * np.dstack((gapExpanded, gapExpanded, gapExpanded)).astype(np.uint8)
		...
		if save == True:
			plt.imsave('Data\\' + str(round(time.time())) + samplename + '_gapColored' + '.tif', gap_colored)
			plt.imsave('Data\\' + str(round(time.time())) + samplename + '_gapPadded' + '.tif', gapPadded_f)
			plt.imsave('Data\\' + str(round(time.time())) + samplename + '_segGapPad' + '.tif', gapPadded_thresh)
...
def resizer(img, scaling_factor, samplename, path, save):
	...
	if save == True:
		resized_path = 'Data\\' + str(round(time.time())) + samplename + '_resizedGapSegpad' + '.tif'
...
def SpanningIP(imgf, imgi, samplename, path, save = True):
	...
	try:
		...
		gap_padding = 200
		gapPadded_f, gapPadded_thresh = gap_finder(imgi, imgf, binary_img, gap_padding, samplename, path=False, save = True)
			
		binary_path = 'Data\\' + str(round(time.time())) + samplename + '_seg' + '.tif'
		#cv2.imwrite( binary_path, binary_img)
		
		gapped_path = 'Data\\' + str(round(time.time())) + samplename + '_gapseg' + '.tif'
		cv2.imwrite(gapped_path, gapPadded_thresh)
		
		scaling_factor = 0.25
		resized_seg, resized_seg_path = resizer(gapped_path, scaling_factor, samplename , path = True, save = True)
