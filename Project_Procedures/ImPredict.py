...
class ImPredict(Core.Procedure):
	def Prepare(self):
		self.name = 'ImPredict'
		self.requirements['initial_file'] = {'source': 'apparatus', 'address': '', 'value': '', 'desc': 'path of initial image file'}
		self.requirements['final_file'] = {'source': 'apparatus', 'address': '', 'value': '', 'desc': 'path of final image file'}        
	...
	def Plan(self):
		...
		if self.apparatus.simulation:
			...
			gapPadded_thresh = SpanML.SpanningIP(ffilename, ifilename, samplename, path = True, save = False)
			...
			class_pred_raw = str(SpanML.VBOW_class(self.model, gapPadded_thresh)) #need a path
			class_pred = class_pred_raw.replace('[','').replace(']','')
		
		self.apparatus.setValue(['information','ProcedureData','SpanningSample','cur_parameters','Class'], class_pred)
