...
def FlexPrinterGUI(apparatus, materials, tools):
    ...
    devices = {}
    devices['gantry']={'type':'Aerotech_A3200_FlexPrinter', 'model':'Flex Printer', 'descriptors':['motion'],'addresstype':'zmqNode', 'address':'procexec'}
    ...
    n = 0
    for materialx in materials:
        devices['pump'+str(n)]={'type':'Nordson_UltimusV', 'COM':'','pressure':0, 'vacuum':0, 'pumprestime':0,'pumpontime':0, 'pumpofftime':0, 'midtime':0, 'addresstype':'zmqNode','descriptors':[], 'address':'procexec'}
        devices['aeropump'+str(n)]={'type':'Nordson_UltimusV_A3200', 'pumpname':'pump'+str(n), 'A3200name':'gantry', 'IOaxis': 'ZZ1', 'IObit':2 ,'pressure':0, 'vacuum':0, 'pumprestime':0,'pumpontime':0, 'pumpofftime':0, 'midtime':0, 'addresstype':'zmqNode','descriptors':[], 'address':'procexec'}
        n += 1
    # Tools
    for tool in tools:
        devices[tool['name']] = {}
        ...
        if tool['type'] == 'Keyence_GT2_A3200':
            # Default Settings
            devices[tool['name']]['addresstype'] = 'zmqNode'
            devices[tool['name']]['address'] = 'procexec'
    ...
    # System
    devices['system'] = {'type': 'System', 'addresstype': 'zmqNode', 'address':'procexec'}
