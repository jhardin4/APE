...
class G(object):
    ...
    def line_span(self,padding,dwell,distances,com_port,pressure,travel_feed):
        """ Prints lines of different length with the same amount of material extruded, by varying feedrate.
        """
        # Switch to relative if in absolute, but keep track of state
        was_absolute = True
        if not self.is_relative:
    ...
    def line_crossing(self,dwell,length,com_port,pressure,travel_feed):
        """ Printing a line that will then be crossed by the nozzle at different feedrates.
        """
        # Switch to relative if in absolute, but keep track of state
        was_absolute = True
        if not self.is_relative:
    ...
    def line_meander(self,width,spacings,com_port):
        """ Prints modified version of a meander with varying spacing with different pressures.
        """
        # Switch to relative if in absolute, but keep track of state
        was_absolute = True
        if not self.is_relative:
        ...
        self.move(x=width/2+2)
        direction = 1
        for space in spacings:
            self.toggle_pressure(com_port)
            self.move(x=direction*(width/2-2))
            self.move(y=space)
            self.move(x=-direction*(width/2-2))
            self.toggle_pressure(com_port)
            self.move(x=-direction*4)
            direction *= -1
    ...
    def export_APE(self):
        ...
        speed_hist = self.speed_history

        # Filling out a list of feedrate history
        temp_speeds = [j[0]-i[0] for i, j in zip(speed_hist[:-1], speed_hist[1:])]
        temp_speeds += [len(position_hist)-speed_hist[-1][0]]
        final_speeds = [i[1] for i, j in zip(speed_hist,temp_speeds) for _ in range(j)]

        # Filling out a list of extruding history
        final_extrudings = []
        extruding_state = False
        for ind in range(len(position_hist)-3):
            if ind+2 in extruding_hist.keys():
                if extruding_hist[ind+2][1]:
                    extruding_state = True
                else:
                    extruding_state = False
            final_extrudings.append(extruding_state)

        # Filling out a list of single line moves history
        keys = ['X','Y','Z']
        final_coords = [[dict(zip(keys, i)),dict(zip(keys, j))] for i,j in zip(position_hist[1:-1],position_hist[2:])]
        
        return final_coords[:-1], final_speeds[1:-1], final_extrudings
    ...
    def view(self, backend='matplotlib', outfile=None, hide_travel=False,color_on=False, nozzle_cam=False, 
             ...
             substrate_dims=[0.0,0.0,-1.0,300,1,300], scene_dims = [1080,720]):
