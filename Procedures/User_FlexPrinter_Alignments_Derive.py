...
class User_FlexPrinter_Alignments_Derive(Procedure):
    ...
    def Plan(self):
        ...
        # Remove the empty strings from pure locations
        toollist[:] = (v for v in toollist if v != '')
        loclist = list(set([n.split('@')[1] for n in measuredlist]))
        if 'initial' in loclist:
        ...
        for axis in alignments['@initial'].keys()^['X','Y']:
            if 'safe' + axis in self.loptions:
                if optionList['safe'+axis]:
                    # These should be @safeZ but I haven't had a chance to fix
        ...
        for tool in toollist:
            ...
            if 'start' in optionList and optionList['start']:
            ...
            if 'cal' in optionList and optionList['cal']:
            ...
            if 'clean' in optionList and optionList['clean']:
