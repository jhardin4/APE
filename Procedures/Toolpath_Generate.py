...
class Toolpath_Generate(Procedure):
    def Prepare(self):
        ...
        self.requirements['generator']={'source':'apparatus', 'address':'', 'value':'', 'desc':'name of generator'}
        self.requirements['target']={'source':'apparatus', 'address':'', 'value':'', 'desc':'where to store the toolpath'}
        self.requirements['dataArgs']={'source':'apparatus', 'address':'', 'value':'', 'desc':'arguments to make parameter structure'}
    ...
    def Plan(self):
        ...
        if dataArgs == '':
            material = self.apparatus.getValue(['information','toolpaths','parameters', 'materialname'])
            dataArgs = [material]
        ...
        if newPara:
            parameters = tempPara
            self.apparatus.setValue(['information','toolpaths','parameters'], tempPara)

        systemname = self.apparatus.findDevice({'descriptors':'system'})
        temptarget = [0]
        self.DoEproc(systemname, 'Run', {'address': generator + '.GenerateToolpath', 'addresstype':'pointer', 'arguments':[parameters, temptarget]})
