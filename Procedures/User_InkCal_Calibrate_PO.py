...
import Procedures.User_InkCal_Measure_PO
import json


class User_InkCal_Calibrate_PO(Procedure):
    def Prepare(self):
        ...
        self.cal_calculation = Procedures.User_InkCal_Calculate_PO(
            self.apparatus, self.executor
        )
        self.cal_measurement = Procedures.User_InkCal_Measure_PO(
            self.apparatus, self.executor
        )
        ...
        if not self.apparatus.getValue(
            ...
            if usecal in ['Y', 'y', 'yes', 'Yes', '']:
                self.apparatus.setValue(
                    ['information', 'materials', material, 'calibrated'], True
                )
                ...
                if newfile in ['Y', 'y', 'yes', 'Yes']:
                    ...
                    message = (
                        'Would you like to make new measurement of ' + material + '?'
                    )
                    options = ['y', 'n']
                    default = 'n'
                    self.useroptions.Do(
