...
class User_InkCal_Calculate_PO(Procedure):
    def Prepare(self):
    ...
    def Plan(self):
        ...
        do_speedcal = self.apparatus.getValue(
            ['information', 'materials', material, 'do_speedcal']
        )
        ...
        if self.apparatus.getValue(
            ...
            if len(file_data) == 1:
                ...
                proj_dweight = (cur_time - initial_time) / (
                    final_time - initial_time
                ) * (final_dweight - initial_dweight) + initial_dweight
            ...
            if do_speedcal:
                self.apparatus.setValue(
                    ['devices', nozzle, 'speed'], targetspeed
                )
            if do_pumpcal:
                self.apparatus.setValue(
                    ['devices', pump, 'pumpon_time'],
                    pumpres_time + 1.5 * trace_height / targetspeed,
                )
