...
class interlace(): #dependent on run_ape, import_ape, deconvolver
    def interlace ( fileorlayer, wholeorpointcloud, usememory ):
        ...
        for i in range ( line_start, line_end ):
            ...
            if wholeorpointcloud == 'whole' or wholeorpointcloud == 'whole pc':
                tempb = deconvolver.deconvolver ( 'auto' ).hotendUI_line ( i, past_line_b )
                if tempb != { 'Empty' : 'Empty' }:
                    past_line_b = tempb
                tempt = deconvolver.deconvolver ( 'auto' ).hotbedUI_line ( i, past_line_t )
                if tempt != { 'Empty' : 'Empty' }:
                    past_line_t = tempt
            if wholeorpointcloud == 'whole':
                tempp = deconvolver.deconvolver ( 'auto' ).interpointUI_line ( i, past_line_p )
                #print ( 'TEMPP', tempp )
                ...
                tempp2 = deconvolver.deconvolver ( 'auto' ).pointUI_line ( i, past_line_p2 )
