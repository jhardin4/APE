...
#        self.thread_number = importape.getthreadcount
    ...
    #returns dictionary
    def point ( call_file, implicitorexplicitaxes, implicitorexplicittravel,  lineorlayer, line_start, line_end, UI_line ):
    ...
    #returns dictionary
    def hotend ( call_file, lineorlayer, line_start, line_end, initialize, UI_line ):
        if lineorlayer == 'layer':
            templine_start = importape.layertoline ( call_file, 'start', line_start, line_end )
            templine_end = importape.layertoline ( call_file, 'end', line_start, line_end )
            line_start = templine_start
    ...
    def extrude ( call_file, lineorlayer, line_start, line_end, UI_line ):
        if lineorlayer == 'layer':
            templine_start = importape.layertoline ( call_file, 'start', line_start, line_end )
            templine_end = importape.layertoline ( call_file, 'end', line_start, line_end )
            line_start = templine_start
    ...
    #returns dictionary
    def hotbed ( call_file, lineorlayer, line_start, line_end, initialize, UI_line ):
        if lineorlayer == 'layer':
            templine_start = importape.layertoline ( call_file, 'start', line_start, line_end )
            templine_end = importape.layertoline ( call_file, 'end', line_start, line_end )
            line_start = templine_start
    ...
    def interpoint ( call_file, implicitorexplicitaxes, implicitorexplicittravel,  lineorlayer, line_start, line_end, UI_line ):
        if lineorlayer == 'layer':
            templine_start = importape.layertoline ( call_file, 'start', line_start, line_end )
            templine_end = importape.layertoline ( call_file, 'end', line_start, line_end )
            line_start = templine_start
        ...
        if line_end == 'end':
            line_end = importape.numberoflines ( call_file, 'end' )
        pastpoint  = nextpoint = [ 0, 0, 0, 0, 0, 0 ]
        toolpathpoint = []
        linepoint = { 'X' : 0, 'Y' : 0, 'Z' : 0, 'E' : 0, 'F' : 0, 'SetE' : 0 }
        tempX = tempY = tempZ = tempE = tempF = tempsetE = 0
        i = 0
        for i in range ( line_start, line_end + 1 ):
            if line_start == line_end:
                tempstr = linecache.getline (call_file, line_start )
    ...
    def hotendUI ( self, initialize ):
        return ( getape.hotend ( self.call_file, self.lineorlayer, self.line_start, self.line_end, initialize, self.UI_line ) )
    
    def hotendUI_line ( self, line ):
        line_start = line
        line_end = line
        return ( getape.hotend ( self.call_file, self.lineorlayer, line_start, line_end, 'no', 'yes' ) [ -1 ] )    
    
    #calles hotbed and loads initialized perameters
    def hotbedUI ( self, initialize ):
        return ( getape.hotbed ( self.call_file, self.lineorlayer, self.line_start, self.line_end, initialize, self.UI_line ) )
        
    def hotbedUI_line ( self, line ):
        line_start = line
        line_end = line
        return ( getape.hotbed ( self.call_file, self.lineorlayer, line_start, line_end, 'no', 'yes' ) [ -1 ] )
    
    def pointUI ( self ):
        return ( getape.point ( self.call_file, self.implicitorexplicitaxes, self.implicitorexplicittravel, self.lineorlayer, self.line_start, self.line_end, self.UI_line ) )
    
    def pointUI_line ( self, line ):
        line_start = line
        line_end = line
        return ( getape.point ( self.call_file, self.implicitorexplicitaxes, self.implicitorexplicittravel, self.lineorlayer, line_start, line_end, 'yes' ) [ -1 ] )
    
    def interpointUI ( self ):
        return ( getape.interpoint ( self.call_file, self.implicitorexplicitaxes, self.implicitorexplicittravel, self.lineorlayer, self.line_start, self.line_end, self.UI_line ) )

    def interpointUI_line ( self , line ):
        line_start = line
        line_end = line
        return ( getape.interpoint ( self.call_file, self.implicitorexplicitaxes, self.implicitorexplicittravel, self.lineorlayer, line_start, line_end, 'yes' ) [ -1 ] )

    def extrudeUI ( self ):
        return ( getape.extrude ( self.call_file, self.lineorlayer, self.line_start, self.line_end, self.UI_line ) )
    
    def extrudeUI_line ( self, line ):
        line_start = line
        line_end = line
        return ( getape.extrude ( self.call_file, self.lineorlayer, line_start, line_end, 'yes' ) [ -1 ] )


#print ( getape ( 'auto' ).hotendUI ( 'yes' ) )
#print ( getape ( 'auto' ).hotbedUI ( 'yes' ) )
#print ( getape ( 'auto' ).pointUI_line ( 1 ) )
#print ( getape ( 'auto' ).pointUI ( ) )
#print ( getape ( 'auto' ).interpointUI_line ( 1 ) )
