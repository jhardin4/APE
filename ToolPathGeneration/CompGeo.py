...
def LineSegIntersect2D (line1, line2, distancetol, angletol):
    ...
    if inter == 'parallel':
        ...
        for dim in line1[0]:
            try:
                ...
                unitvector[dim] = 1
                angletest = smallangle_lines(line1, [ {dimlist[0]:0, dimlist[1]:0} , unitvector ])
                ...
                if almostequal(angletest, 0, angletol):
                    ...
                    for otherdim in line1[0]:
                        if otherdim != dim:
                            if almostequal(line1[0][otherdim],line2[0][otherdim],distancetol):
                                inter = 'colinear'
                            else:
                                return 'none'
            except:
    ...
    if inter == 'parallel':
        try:
            ...
            dimlist = list(line1[0].keys())
            mpara = (line1[0][dimlist[1]]-line1[1][dimlist[1]])/(line1[0][dimlist[0]]-line1[1][dimlist[0]])
            #bpara = line1[0][1] - mpara*line1[0][0]
            #Construct perpenticular line through center of line1
            #mperp * x + bperp = y 
            mperp = -1/mpara
            center1 = {dimlist[0]:(line1[0][dimlist[0]]+line1[1][dimlist[0]])/2 , dimlist[1]:(line1[0][dimlist[1]]+line1[1][dimlist[1]])/2 }
            bperp = center1[dimlist[1]] - mperp*center1[dimlist[0]]
            #pick two points on the line to define it, say x= 0 and x =1
            perpline = [{dimlist[0]:0,dimlist[1]:bperp},{dimlist[0]:1,dimlist[1]:1*mperp + bperp}]
    ...
    if inter == 'colinear':
        ...
        ptlist.sort(key=lambda x: (x[0][dimlist[1]]-lowpoint[dimlist[1]])**2+(x[0][dimlist[0]]-lowpoint[dimlist[0]])**2)
    ...
    for point in line1:
        ...
        if (min(linex)-distancetol)<=inter[dimlist[0]]<=(max(linex)+distancetol) and (min(liney)-distancetol)<=inter[dimlist[1]]<=(max(liney)+distancetol):
            inter1 = 1
...
def pointinregion (point, region, distancetol, angletol):
    ...
    if (point[dimx] < box[0]) or (point[dimx] > box [1]) or (point[dimy] < box[2]) or (point[dimy] > box [3]):
    ...
    #horizontal line
    rays.append( [{dimx:frame[0],dimy:point[dimy]},{dimx:frame[1],dimy:point[dimy]}] )
    
    #line that is rotated by e degrees (y = mx + c)
    m = math.tan(math.exp(1)*math.pi/180)
    c = point[dimy] - m * point[dimx]
    rays.append([{dimx:frame[0], dimy:m*frame[0] + c},{dimx:frame[1],dimy:m*frame[1]+c}])
