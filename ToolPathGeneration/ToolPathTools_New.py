...
import CompGeo as cg  # Used for testing this file.  Comment out otherwise.
import json

# -----GOAL-----
# The goal of these tools are to make it easier to make toolpaths of the
# structure used by APE.  APE toolpaths are list with two kinds of elements.
# Motion elements define a start point, end point, and the materials between
# them.  'material' is a historical term.  In practice is should be tool. It
# is also important to note that these are strictly linear at this time.
# Other motions types can be added using the custome parse statement.
...
# Typically, people like to start with with a function that generates a list
# of ordered points.  Sometimes, there can be multiple list of points
# generated, where each is its own motion (collection of uninterrupted motion
# elements).
...
def nPt2ToolPath(Point_array, material):
    ...
    # material is the tool that will be used for tracinging the points in
    # Point_Array.

    # Initialize the toolpath
    toolpath = []
    # Go through each element of Point_array
    for n in range(0, len(Point_array)):
        # if element is dict assume it is a point
        ...
        # if element is list then call function again
            ...
            # Add the resulting toolpath to the current one
            toolpath = [*toolpath, *sub_toolpath]
    return toolpath
...
def toolpcut(toolpath, boundry, keepoutside, distancetol, angletol):
    ...
    for line in range(0,len(toolpath)):
        ...
        if 'startpoint' in linekeys:
            ...
            inregion1 = cg.pointinregion(toolpath[line]['startpoint'], boundry, distancetol, angletol)
            inregion2 = cg.pointinregion(toolpath[line]['endpoint'], boundry, distancetol, angletol)
            ...
            for b in range(0,len(boundry)):
                ...
                toolpathline = [toolpath[line]['startpoint'] , toolpath[line]['endpoint'] ]
                intest = cg.LineSegIntersect2D(toolpathline, borderwall, distancetol, angletol)
            ...
            if len(intersections) != 0:
                ...
                for w in range(0, len(intersections)):
                    ...
                    if cg.ptalmostequal(toolpath[line]['startpoint'], intersections[w], distancetol):
                        #remove intersections from copy1 which are identical to start point
                        del(tempintest1[w-cuts1])
                        cuts1 = cuts1 + 1
                    if cg.ptalmostequal(toolpath[line]['endpoint'], intersections[w], distancetol):
            ...
            if inregion1 == keepoutside and inregion2 == keepoutside:    
                ...
                #remove motions satisfying condition 3 (only applied when keeping inside)
                    ...
                    if (point1all == 1 and point2all == 0 ) or (point1all == 0 and point2all == 1 ):
                        del(temppath1[line-cuts])
                        cuts = cuts + 1
    ...
    for line in range(0,len(temppath1)):
        ...
        for b in range(0,len(boundry)):
            ...
            intest = cg.LineSegIntersect2D(temppath1line, borderwall, distancetol, angletol)
            #Construct list of intersections, both normal and colinear
            if intest != 'none':
                intersections.append(intest)
        #Break up motion across boundry
        #check if motion intersects
        inregion1 = cg.pointinregion(temppath1[line]['startpoint'], boundry, distancetol, angletol)
        inregion2 = cg.pointinregion(temppath1[line]['endpoint'], boundry, distancetol, angletol)
        ...
        if intersections != []:   
            ...
            intersections.sort(key = lambda x:(x[dimlist[0]]-temppath1[line]['startpoint'][dimlist[0]])**2 + (x[dimlist[1]]-temppath1[line]['startpoint'][dimlist[1]])**2)
            ...
            if inregion1==True or cg.ptalmostequal(intersections[0], temppath1[line]['startpoint'], distancetol):
            ...
            for e in range(0,len(intersections)):
                if (not cg.ptalmostequal(intersections[e], temppath1[line]['startpoint'], distancetol)) and (not cg.ptalmostequal(intersections[e], temppath1[line]['endpoint'], distancetol)):
            ...
            pointlist.sort(key = lambda x:(x[dimlist[0]]-temppath1[line]['startpoint'][dimlist[0]])**2 + (x[dimlist[1]]-temppath1[line]['startpoint'][dimlist[1]])**2)
            ...
            for i in range(0,len(pointlist)):
                ...
                if i % 2 == 1:
                    ...
                    if i == 1:
                        ...
                        tempmotion = {'startpoint':pointlist[i-1] , 'endpoint':pointlist[i], 'material':temppath2[line+motionadds]['material']}
    ...
    for p in range(0,len(cleanedpath1)):
        ...
        for q in range(1,len(cleanedpath1[p])):
            if len(cleanedpath2) > 1:
                if not cg.ptalmostequal (cleanedpath2[p+padds][t-1][1] , cleanedpath2[p+padds][t][0] , distancetol):
                    #construct residual
                    temppath = []
...
def toolpathplotter3D (toolpath):
    ...
    for p in range(0,len(toolpath)):
        if not(len(toolpath[p])==3 and len(toolpath[p][0])==3 and len(toolpath[p][1])==3):
            temp = temp +1 
    ...
    if depth == 1:
        for p in range(0,len(toolpath)):
            for q in range(0,len(toolpath[p])):
                ...
                if not(len(toolpath[p][q])==3 and len(toolpath[p][q][0])==3 and len(toolpath[p][q][1])==3):
                    temp = temp +1 
        ...
        for p in range(0,len(toolpath)):
            for q in range(0,len(toolpath[p])):
                for r in range(0,len(toolpath[p][q])):
                    ...
                    if not(len(toolpath[p][q][r])==3 and len(toolpath[p][q][r][0])==3 and len(toolpath[p][q][r][1])==3):
                        temp = temp +1 
        depth = depth + math.ceil(temp/total)
...
def buildexporter_anigif (toolpath, images, rotations, materials):
    ...
    for p in range(0,images):
        ...
        ax.set_xlim([-5,60])
        ax.set_ylim([-15,50])
        plottoolpaths(toolpath[:math.floor(0.5*p*framelength)], materials2, newfigure=False)
        plottoolpaths(toolpath[math.floor(0.5*p*framelength):math.floor(0.9*p*framelength)], materials1, newfigure=False)
        plottoolpaths(toolpath[math.floor(0.9*p*framelength):p*framelength], materials, newfigure=False)  
...
def pntlabel2D(points, index=[], newplot = True):
    ...
    for m in range(0,len(points)):
        for n in range(0,len(points[m])):
            label = '('+str(m)+','+ str(n)+')'
            plt.annotate(label, xy = (points[m][n][0], points[m][n][1]), xytext = (7, -3), textcoords = 'offset points') 
...
def parse_endofmotion(toolpath, disttol):
    ...
    for l in range(0,len(toolpath)):
        ...
        if 'startpoint' in linekeys:
            ...
            elif not cg.ptalmostequal(prevmotion['endpoint'], toolpath[l]['startpoint'], disttol):
    ...
    for l in range(0,len(toolpath)):
        ...
        if 'parse' in linekeys:
            if toolpath[l]['parse'] == 'endofmotion':
                ...
                parsed_TP.insert(prev_endofmotion+parseadds+1,{'parse':'startofmotion', 'motion':startmotion})
                parseadds += 1
                prev_endofmotion = l
        ...
        if prevline == []:
            ...
            if 'startpoint' in plinekeys:
                if prevline['material'] != line['material']:
                    parsed_TP.append({'parse':'changemat', 'startmotion':prevline, 'endmotion':line})
        prevline = line
...
if __name__ == "__main__":
    pointlist = []
    pointlist.append({'X': 0, 'Y': 0, 'Z': 0})
    pointlist.append({'X': 0, 'Y': 10, 'Z': 0})
    pointlist.append({'X': 10, 'Y': 10, 'Z': 0})
    pointlist.append({'X': 10, 'Y': 0, 'Z': 0})
    pointlist.append({'X': 0, 'Y': 0, 'Z': 0})

    toolpath = nPt2ToolPath(pointlist, 'test')
