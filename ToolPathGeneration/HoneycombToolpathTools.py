...
def basearray(apara, bpara, cpara, mmax, nmax):
    ...
    for m in range( 0 , mmax ):
        ...
        for n in range( 0, nmax ):
            point_array_line.append([ ( m * bpara / 2 ) , (n*((2*apara+2*cpara)/2)+(-2*cpara)/4+(-1)**(n-m)*((2*cpara)/4)) ])
...
def throughpath(border, tlayers, ltl, zpos, material, matslide):
    ...
    center = { 'X':sum([border[n]['X'] for n in range(0,len(border))])/len(border),'Y':sum([border[n]['Y'] for n in range(0,len(border))])/len(border)}
    through_path.append({'startpoint': {'X':center['X'],'Y':center['Y'],'Z':through_z}, 'endpoint':{'X':border[0]['X'], 'Y':border[0]['Y'], 'Z':through_z}, 'material':material})
    bordercycle = border[:]
    bordercycle.append(border[0])
    for t in range(0,tlayers):
        #run around the border for all layers
        for p in range(0,len(border)):
            startpoint = {'X':bordercycle[p]['X'], 'Y':bordercycle[p]['Y'], 'Z':through_z}
            endpoint = {'X':bordercycle[p+1]['X'], 'Y':bordercycle[p+1]['Y'], 'Z':through_z}
            through_path.append({'startpoint':startpoint, 'endpoint':endpoint, 'material':material})
        #move up for each layer but the last
        if t !=(tlayers-1):
                through_path.append({'startpoint':{'X':border[0]['X'], 'Y':border[0]['Y'], 'Z':through_z}, 'endpoint':{'X':border[0]['X'], 'Y':border[0]['Y'], 'Z':through_z + ltl/tlayers}, 'material':material})
    ...
    for t in range(0,3):
        for p in range(0,len(border)):
            startpoint = {'X':bordercycle[p]['X'], 'Y':bordercycle[p]['Y'], 'Z':through_z}
            endpoint = {'X':bordercycle[p+1]['X'], 'Y':bordercycle[p+1]['Y'], 'Z':through_z}
            through_path.append({'startpoint':startpoint, 'endpoint':endpoint, 'material':matslide})
...
def hexpt2toolpath(point_matrix, material, materialslide):
    ...
    for n in range(0,len(point_matrix[0])):
        for m in range(1, len(point_matrix)):
            toolpath.append({'startpoint':tpt.list2XY(point_matrix[m-1][n]),'endpoint':tpt.list2XY(point_matrix[m][n]),'material':material})
    ...
    for k in range(0,math.floor(len(point_matrix)/2)):
        ...
        while m < len(point_matrix) and n < len(point_matrix[0]):
            if t==1:
                toolpath.append({'startpoint':tpt.list2XY(point_matrix[m+1][n]),'endpoint':tpt.list2XY(point_matrix[m][n]),'material':materialslide})
                n = n+1
                t = t+1
            elif t==2:
                toolpath.append({'startpoint':tpt.list2XY(point_matrix[m][n-1]),'endpoint':tpt.list2XY(point_matrix[m][n]),'material':material})
                m = m+1
                t = t+1
            elif t==3:
                toolpath.append({'startpoint':tpt.list2XY(point_matrix[m-1][n]),'endpoint':tpt.list2XY(point_matrix[m][n]),'material':materialslide})
...
def DistXYundulation (pointarray, apara, bpara, cpara, magx, magy):
    ...
    for m in range( 0 , len(point_darray)):
        for n in range( 0, len(point_darray[m]) ):
            point_darray[m][n] = [ point_darray[m][n][0] + magx * math.sin(2*math.pi*point_darray[m][n][0] / 3 / bpara + phasex ) , point_darray[m][n][1] + magy * math.sin(2*math.pi*point_darray[m][n][1] / (2 * apara + 2 *cpara) + phasey ) ]
    return(point_darray)
...
def hexcellborder (hexptarray, mint, nint, offset):
    ...
    connectionlines.append([hexptarray[mint+2][nint+1],hexptarray[mint+3][nint+1]])
    connectionlines.append([hexptarray[mint+1][nint+1],hexptarray[mint+1][nint+2]])
    ...
    for l in range(0,len(connectionlines)):
        if connectionlines[l][1][0] == connectionlines[l][0][0]:
            points.append( {'X':connectionlines[l][1][0] , 'Y':(connectionlines[l][1][1]-connectionlines[l][0][1])/abs(connectionlines[l][1][1]-connectionlines[l][0][1]) * offset +connectionlines[l][0][1]})
        else:
            mtemp = (connectionlines[l][1][1]-connectionlines[l][0][1])/(connectionlines[l][1][0]-connectionlines[l][0][0])
            xtemp = offset/(1+mtemp**2)**0.5 * (connectionlines[l][1][0]-connectionlines[l][0][0])/abs(connectionlines[l][1][0]-connectionlines[l][0][0])+connectionlines[l][0][0]
            ytemp = (xtemp-connectionlines[l][0][0]) * mtemp + connectionlines[l][0][1]
...
def shiftarray (array, shiftx, shifty):
    ...
    for m in range(0,len(array)):
        for n in range(0,len(array[0])):
            sarray[m][n]=[ array[m][n][0] - array[shiftx][shifty][0] , array[m][n][1] - array[shiftx][shifty][1] ]
...
def hexpt2toolpath_ground(point_matrix, material):
    ...
    for p in range(0,math.floor(maxm/2)):
        ...
        while newn<maxn and newm<maxm:
            toolpath.append({'startpoint':tpt.list2XY(point_matrix[m][n]),'endpoint':tpt.list2XY(point_matrix[newm][newn]),'material':material})
            m = newm
            n = newn
            if alternation2==-1:
                newm = m + 2
